...
def parse_roster(html_content):
    ...
    for row in rows:
        ...
        # Note: dates are indices 0..N in our dates list, but columns are 1..N+1 in the row (0 is role)
        # We want the first date, so column index 1.
        if len(cells) > 1:
...
def generate_whatsapp_link(date, open_positions):
    if not open_positions:
        return "Alle Positionen sind besetzt! \U0001F389", None
    ...
    # According to StackOverflow (https://stackoverflow.com/questions/64215662/whatsapp-click-to-chat-with-emojis),
    # wa.me redirects often break emoji encoding.
    # The solution is to use api.whatsapp.com/send directly with standard encoding.
